...
public class GUI_Client extends JFrame implements CoapClient{
	...
	JButton btn_observe = new JButton("OBSERVE"); 	// Observe button 생성
	...
	public GUI_Client (String serverAddress, int serverPort) {
		...
		btn_observe.setBounds(240, 670, 100, 50);
		btn_post.setBounds(350, 670, 100, 50);
		btn_delete.setBounds(460, 670, 100, 50);
		...
		btn_observe.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				request.setObserveOption(0); // 시작 sequence number 설정
				displayRequest(request);
				clientChannel.sendMessage(request);
			}
		});
	...
	public void onResponse(CoapClientChannel channel, CoapResponse response) {
		...
		if(Encoder.ByteToString(response.getToken()).equals("obToken")) {
			String state = Encoder.ByteToString(response.getPayload());
			this.control_led(state);
		}
	}
	
	public void control_led(String state) {
	...
	public static void main(String[] args){
		//프레임 열기
		GUI_Client gui = new GUI_Client("fe80::9805:dfab:8fec:1f3c", CoapConstants.COAP_DEFAULT_PORT);
	}
